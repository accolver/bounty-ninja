...
#### Scenario: User publishes a task event within cooldown

- **WHEN** a user successfully publishes a Kind 37300 task event
...
- **WHEN** a user published a Kind 37300 task event 31 seconds ago (cooldown:
  30s)
...
- **WHEN** a user pledges to a task (Kind 73002)
- **THEN** the cooldown SHALL be 10 seconds
- **AND** the user SHALL be able to pledge to a different task after 10
  seconds

### Requirement: Rate Limiter UI Integration

All publish-triggering UI elements (TaskForm submit, PledgeForm submit,
...
- **WHEN** a user has just published a task and the 30-second cooldown is
  active
- **THEN** the "Create Task" button on `/task/new` SHALL be disabled
...
- **WHEN** a user publishes a task event and immediately refreshes the page
- **THEN** the rate limiter state SHALL be cleared
- **AND** the user SHALL be able to publish another task event immediately

### Requirement: Rate Limiter Bypass for Replaceable Events

Parameterized replaceable events (Kind 37300 task updates) that update an
existing event (same `d` tag) SHALL have a reduced cooldown of 10 seconds
instead of the standard 30 seconds, since updates to existing tasks are a
legitimate rapid-edit use case.

#### Scenario: Task update has reduced cooldown

- **WHEN** a user edits an existing task (publishes a Kind 37300 with the same
  `d` tag)
- **THEN** the cooldown for the next edit of that same task SHALL be 10
  seconds
- **AND** creating a new task (different `d` tag) SHALL still use the
  30-second cooldown

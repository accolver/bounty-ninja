...
- [x] 1.1 Update `BountyStatus` type in `src/lib/bounty/types.ts` — add
...
- [x] 1.2 Update `BountyDetail` interface in `src/lib/bounty/types.ts` — replace
      `payout: Payout | null` with `payouts: Payout[]`. Empty array means no
      payouts.
- [x] 1.3 Update `EscrowState` type in `src/lib/cashu/types.ts` — change from
      `'locked' | 'claimed' | 'refunded' | 'expired'` to
      `'locked' | 'released' | 'reclaimed' | 'expired'`. Update JSDoc to reflect
      pledger-controlled semantics.
- [x] 1.4 Remove unused types from `src/lib/cashu/types.ts` — remove
...
- [x] 2.1 Update `deriveBountyStatus` in `src/lib/bounty/state-machine.ts` — add
...
- [x] 2.2 Update `src/tests/unit/state-machine.test.ts` — add tests for:
...
- [x] 3.1 Rewrite `createPledgeToken` in `src/lib/cashu/escrow.ts` — rename
...
- [x] 3.2 Add `releasePledgeToSolver` function in `src/lib/cashu/escrow.ts` —
      ...
      deduction.
- [x] 3.3 Simplify `reclaimExpiredPledge` → rename to `reclaimPledge` in
...
- [x] 3.4 Remove deprecated functions from `src/lib/cashu/escrow.ts` — delete:
...
- [x] 3.5 Rewrite `src/tests/unit/escrow.test.ts` — full rewrite for
...
- [x] 4.1 Update `parsePayout` in `src/lib/bounty/helpers.ts` — change
...
- [x] 4.2 Update `parseBountyDetail` in `src/lib/bounty/helpers.ts` — (a)
...
- [x] 4.3 Update `src/tests/unit/helpers.test.ts` — update `parsePayout` tests:
...
- [x] 5.1 Update `src/lib/stores/bounty-detail.svelte.ts` — change `payout`
...
- [x] 5.2 Update `src/lib/stores/bounties.svelte.ts` — handle
...
- [x] 6.1 Update `src/lib/components/pledge/PledgeForm.svelte` — change
...
- [x] 6.2 Update `src/lib/components/pledge/ReclaimAlert.svelte` — import
...
- [x] 6.3 Update `src/lib/components/pledge/PledgeItem.svelte` — optionally show
...
- [x] 7.1 Redesign `src/lib/components/voting/PayoutTrigger.svelte` — full
...
- [x] 7.2 Update `src/lib/components/voting/SolverClaim.svelte` — handle
...
- [x] 7.3 Update `src/lib/components/voting/VoteResults.svelte` — after
...
- [x] 7.4 Update `src/lib/components/bounty/BountyStatusBadge.svelte` — add
...
- [x] 8.1 Update `src/lib/components/bounty/BountyDetailView.svelte` — (a)
...
- [x] 9.1 Rewrite `src/tests/integration/payment-pipeline.test.ts` — full
...
- [x] 9.2 Update `src/tests/integration/pledge-flow.test.ts` — update lock
      ...
      assertions.
- [x] 9.3 Update `src/tests/integration/bounty-store.test.ts` — add tests for
...
- [x] 10.1 Update `src/tests/unit/blueprints.test.ts` — verify `payoutBlueprint`
...
- [x] 10.2 Update `src/tests/unit/tag-validator.test.ts` — minor description
...
- [x] 11.1 Run `bun run test` — all unit and integration tests must pass (expect
      ~578+ tests with updates). Fix any regressions.
- [x] 11.2 Run `bunx svelte-check --threshold error` — zero TypeScript or Svelte
      compilation errors.
- [x] 11.3 Run `bun run build` — production build succeeds with no errors.
- [x] 11.4 Manually verify the complete lifecycle in the UI: create bounty →

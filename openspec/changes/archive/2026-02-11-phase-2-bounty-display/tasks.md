# Phase 2 — Bounty Display: Tasks

## 1. Data Models & Constants

- [x] 1.1 Verify/extend `src/lib/bounty/kinds.ts` — export `BOUNTY_KIND`
      (37300), `SOLUTION_KIND` (73001), `PLEDGE_KIND` (73002), `VOTE_KIND`
      (1018), `PAYOUT_KIND` (73004) as named numeric constants
- [x] 1.2 Create `src/lib/bounty/types.ts` — define `BountyStatus` union type
      (`"draft" | "open" | "in_review" | "completed" | "expired" | "cancelled"`)
- [x] 1.3 In `src/lib/bounty/types.ts` — define `Bounty` interface with fields:
...
- [x] 1.4 In `src/lib/bounty/types.ts` — define `Pledge` interface with fields:
      `event`, `id`, `pubkey`, `bountyAddress`, `amount`, `cashuToken`,
      `mintUrl`, `createdAt`, `message`
- [x] 1.5 In `src/lib/bounty/types.ts` — define `Solution` interface with
      fields: `event`, `id`, `pubkey`, `bountyAddress`, `description`,
      `antiSpamToken`, `antiSpamAmount`, `deliverableUrl` (string | null),
      `createdAt`, `voteWeight`
- [x] 1.6 In `src/lib/bounty/types.ts` — define `Vote` interface with fields:
      `event`, `id`, `pubkey`, `bountyAddress`, `solutionId`, `choice`
      (`"approve" | "reject"`), `pledgeAmount`, `weight`, `createdAt`
- [x] 1.7 In `src/lib/bounty/types.ts` — define `Payout` interface with fields:
      `event`, `id`, `pubkey`, `bountyAddress`, `solutionId`, `solverPubkey`,
      `amount`, `cashuToken`, `createdAt`
- [x] 1.8 In `src/lib/bounty/types.ts` — define `BountyDetail` interface
      extending `Bounty` with `pledges`, `solutions`, `votesBySolution`
      (Map<string, Vote[]>), `payout` (Payout | null), `creatorProfile` (object
      | null)
- [x] 1.9 In `src/lib/bounty/types.ts` — define `BountySummary` interface with
...
## 3. Bounty State Machine

- [x] 3.1 Create `src/lib/bounty/state-machine.ts` — implement
      `deriveBountyStatus(bountyEvent, pledges, solutions, payouts, deleteEvents, now?): BountyStatus`
...
- [x] 3.3 Handle expiration edge cases: missing tag = never expires, malformed
      tag = never expires, completed bounty past deadline = still completed (not
      expired)

## 4. Voting System

- [x] 4.1 Create `src/lib/bounty/voting.ts` — implement
...
- [x] 4.2 In `src/lib/bounty/voting.ts` — implement
...
- [x] 5.1 Create `src/lib/bounty/filters.ts` — implement
      `bountyListFilter(limit?: number): Filter` returning
      `{ kinds: [BOUNTY_KIND], limit: limit ?? 50 }` using kind constants from
      `./kinds`
- [x] 5.2 In `src/lib/bounty/filters.ts` — implement
      `pledgesForBountyFilter(bountyAddress: string): Filter` returning
      `{ kinds: [PLEDGE_KIND], "#a": [bountyAddress] }`
- [x] 5.3 In `src/lib/bounty/filters.ts` — implement
      `solutionsForBountyFilter(bountyAddress: string): Filter` returning
      `{ kinds: [SOLUTION_KIND], "#a": [bountyAddress] }`
- [x] 5.4 In `src/lib/bounty/filters.ts` — implement
      `votesForBountyFilter(bountyAddress: string): Filter` returning
      `{ kinds: [VOTE_KIND], "#a": [bountyAddress] }`
- [x] 5.5 In `src/lib/bounty/filters.ts` — implement
      `payoutForBountyFilter(bountyAddress: string): Filter` returning
      `{ kinds: [PAYOUT_KIND], "#a": [bountyAddress] }`
- [x] 5.6 In `src/lib/bounty/filters.ts` — implement
      `bountyByAuthorFilter(pubkey: string): Filter` returning
      `{ kinds: [BOUNTY_KIND], authors: [pubkey] }`
- [x] 5.7 In `src/lib/bounty/filters.ts` — implement
      `searchBountiesFilter(query: string, limit?: number): Filter` returning
      `{ kinds: [BOUNTY_KIND], search: query, limit: limit ?? 20 }`
- [x] 5.8 Verify all filter functions import and use kind constants from
      `src/lib/bounty/kinds.ts` — no hardcoded numeric literals

## 6. Unit Tests — Domain Logic

- [x] 6.1 Create `src/tests/unit/helpers.test.ts` — test `parseBountySummary`
...
- [x] 6.2 In `src/tests/unit/helpers.test.ts` — test `parsePledge` extracts
      bounty address from `a` tag, `parseSolution` extracts deliverable URL from
      `r` tag, `parseVote` extracts choice and solution reference
- [x] 6.3 In `src/tests/unit/helpers.test.ts` — test `parseBountyDetail`
...
- [x] 7.1 Create `src/lib/nostr/loaders/bounty-loader.ts` — export a
...
- [x] 7.2 Create `src/lib/nostr/loaders/pledge-loader.ts` — export
      `createPledgeLoader(bountyAddress: string): Subscription` for Kind 73002
      events filtered by `#a` tag; adds events to singleton `EventStore`
- [x] 7.3 Create `src/lib/nostr/loaders/solution-loader.ts` — export
      `createSolutionLoader(bountyAddress: string): Subscription` for Kind 73001
      events filtered by `#a` tag; adds events to singleton `EventStore`
- [x] 7.4 Create `src/lib/nostr/loaders/vote-loader.ts` — export
      `createVoteLoader(bountyAddress: string): Subscription` for Kind 1018
      events filtered by `#a` tag; adds events to singleton `EventStore`
...
- [x] 8.1 Create `src/lib/stores/bounties.svelte.ts` — implement
      `BountyListStore` class with `$state` fields (`#items`, `#loading`,
      `#error`), getters (`items`, `loading`, `error`, `popular`), subscribing
      to `EventStore.timeline()` for Kind 37300 events
- [x] 8.2 In `BountyListStore` — parse incoming events via
      `parseBountySummary()`, set `loading = false` on first emission, set
      `error` on Observable error
- [x] 8.3 In `BountyListStore` — implement `popular` getter returning items
      sorted by `totalPledged` descending
- [x] 8.4 Export singleton `bountyList` instance from
      `src/lib/stores/bounties.svelte.ts`
- [x] 8.5 Create `src/lib/stores/bounty-detail.svelte.ts` — implement
      `BountyDetailStore` class with `$state` fields (`#bounty`, `#loading`,
      `#error`), getters (`bounty`, `loading`, `error`)
- [x] 8.6 In `BountyDetailStore` — implement
      `load(bountyAddress, kind, pubkey, dTag)` that initiates subscriptions for
      bounty, pledges, solutions, votes, and payouts; composes `BountyDetail`
      via `parseBountyDetail()`
- [x] 8.7 In `BountyDetailStore` — implement `destroy()` that unsubscribes all
...
## 10. Bounty Display Components

- [x] 10.1 Create `src/lib/components/bounty/BountyStatusBadge.svelte` — accept
      `status: BountyStatus` via `$props()`; map to colors (draft→`--muted`,
...
- [x] 10.2 Create `src/lib/components/bounty/BountyTags.svelte` — accept
      `tags: string[]` via `$props()`; render horizontal list of tag pills;
      render nothing when array is empty
- [x] 10.3 Create `src/lib/components/bounty/BountyTimer.svelte` — accept
...
- [x] 10.4 Create `src/lib/components/bounty/BountyCard.svelte` — accept
      `BountySummary` via `$props()`; compose `BountyStatusBadge`, `BountyTags`,
      `SatAmount`, `TimeAgo`; display title (2-line clamp), solution count,
      reward amount; wrap in `<a>` linking to `/bounty/{naddr}` (encode via
      `nip19.naddrEncode()`); ensure keyboard accessibility with visible focus
      ring
- [x] 10.5 Create `src/lib/components/bounty/BountyDetail.svelte` — accept
      `BountyDetail` via `$props()`; compose `BountyStatusBadge`, `Markdown`
      (description), `BountyTags`, `BountyTimer`, `SatAmount` (pledged +
...
- [x] 14.2 Update `src/routes/+page.svelte` — import and consume singleton
      `bountyList` from `src/lib/stores/bounties.svelte.ts`; show
      `LoadingSpinner` while `bountyList.loading`, `EmptyState` when items
      empty, responsive grid of `BountyCard` components (1 col mobile, 2 col
      tablet, 3 col desktop) sorted by `totalPledged` descending via
      `bountyList.popular`

## 15. Bounty Detail Route

- [x] 15.1 Create `src/routes/bounty/[naddr]/+page.ts` — decode `params.naddr`
      via `nip19.decode()`, validate type is `"naddr"`, extract
      kind/pubkey/identifier/relays, return bounty address; throw 400 for wrong
      NIP-19 type, 404 for decode failure; set `ssr = false`,
      `prerender = false`
- [x] 15.2 Create `src/routes/bounty/[naddr]/+page.svelte` — instantiate
      `BountyDetailStore`, call `load()` with decoded data from load function;
      show `LoadingSpinner` while loading, error message on error,
      `BountyDetail` component when data available; call `destroy()` on
...
- [x] 16.2 Create `src/routes/profile/[npub]/+page.svelte` — fetch Kind 0
      profile metadata and Kind 37300 bounties by author; display avatar, name
      (fallback to truncated npub), list of `BountyCard` components; show
      loading and empty states

## 17. Navigation & Layout Updates

- [x] 17.1 Update `src/lib/components/layout/Header.svelte` — add navigation
      links to home page and any relevant bounty pages

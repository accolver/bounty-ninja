...
Phase 2 introduces all task-specific data models, domain logic, reactive
stores, and the complete read-only UI. After Phase 2, users can browse tasks,
view details with pledges/solutions/votes, and navigate profiles — but cannot
yet create, fund, or interact with tasks (write operations are deferred to
Phase 3).

## Goals

- Define TypeScript interfaces for all five task event kinds (37300, 73001,
  73002, 1018, 73004) plus aggregate types (`TaskDetail`, `TaskSummary`)
- Implement pure-function tag parsing that extracts typed data from raw
  `NostrEvent` tags
- Implement the task lifecycle state machine that derives status from related
  events at read time
- Implement linear weighted voting calculation with quorum logic
- Build Nostr filter functions for all task-related queries
...
- Build all read-only UI components: task cards, detail view, status badges,
...
- Implement the home page with a task card grid sorted by total pledged sats
- Implement the task detail page with naddr NIP-19 routing
...
- **Event publishing** — `EventFactory`, blueprints, and the publish flow are
  Phase 3
- **Interactive components** — `TaskForm`, `PledgeForm`, `PledgeButton`,
...
### D2: EventStore.timeline() for reactive task queries

The `TaskListStore` subscribes to
`eventStore.timeline({ kinds: [TASK_KIND] })` which returns an Observable that
emits whenever the set of matching events changes. This provides automatic
reactivity — when new task events arrive from relays, the store updates
without manual polling.

For the `TaskDetailStore`, we use multiple timeline subscriptions (one per
related event kind, filtered by task address via `#a` tag) and compose them
into a single `TaskDetail` object.

### D3: Tag-based parsing from raw NostrEvent

All task data is extracted from raw `NostrEvent` tags using pure helper
functions (`parseTaskSummary()`, `parsePledge()`, etc.). This keeps the domain
...
`TaskStatus` is computed by `deriveTaskStatus()` from the task event and
its related events (pledges, solutions, payouts, deletes). It is never stored as
a tag on the task event. This ensures the displayed status always reflects the
...
Task detail pages use `/task/[naddr]` where `naddr` is a NIP-19 encoded
...
all three plus relay hints, making task URLs self-contained and shareable.

### D6: Sanitized markdown rendering

Task descriptions and solution content are markdown. We render them via a
...
**Mitigation:** The store layer (`tasks.svelte.ts`,
`task-detail.svelte.ts`) acts as an adapter between Applesauce and the UI. If
...
**Risk:** Popular tasks could accumulate hundreds of pledges, solutions, and
...
**Risk:** If related events (pledges, solutions) arrive after the task event,
the derived status may briefly show an incorrect state (e.g., "draft" when
pledges exist but haven't been fetched yet).

**Mitigation:** The `TaskDetailStore` re-derives status whenever any related
event subscription emits. The `TaskListStore` shows `totalPledged: 0` and

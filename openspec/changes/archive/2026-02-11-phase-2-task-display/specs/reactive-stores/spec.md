...
### Requirement: TaskListStore — Class-Based Svelte 5 Rune Store

The module at `src/lib/stores/tasks.svelte.ts` SHALL export a singleton
`TaskListStore` class that bridges Applesauce `EventStore` RxJS Observables to
...
class TaskListStore {
  ...
  get popular(): TaskSummary[]; // sorted by totalPledged descending
}

export const taskList: TaskListStore;
```

#### Scenario: Store initializes with loading state

- **WHEN** the `TaskListStore` is first instantiated
...
- **WHEN** the `EventStore.timeline()` Observable emits an array of Kind 37300
  events
- **THEN** `items` MUST be updated with `TaskSummary` objects parsed via
  `parseTaskSummary()`
...
#### Scenario: Popular tasks sorted by total pledged
...
- **WHEN** `taskList` is imported from multiple modules
- **THEN** the same instance MUST be returned (singleton pattern)

---

### Requirement: TaskDetailStore — Class-Based Svelte 5 Rune Store

The module at `src/lib/stores/task-detail.svelte.ts` SHALL export a class
`TaskDetailStore` that manages the full detail view for a single task,
...
kind) and compose them into a `TaskDetail` object. The store MUST use `$state`
for all reactive fields.

```typescript
class TaskDetailStore {
  #task: TaskDetail | null; // $state
  #loading: boolean; // $state
  #error: string | null; // $state
  ...
  load(taskAddress: string, kind: number, pubkey: string, dTag: string): void;
  destroy(): void;
}
```

#### Scenario: Store loads task by address

- **WHEN** `load("37300:pubkey:task-1", 37300, "pubkey", "task-1")` is
  called
- **THEN** the store MUST initiate subscriptions for the task event, its
  pledges (Kind 73002), solutions (Kind 73001), votes (Kind 1018), and payouts
  (Kind 73004) filtered by the task address

#### Scenario: Store composes TaskDetail from related events

- **WHEN** events arrive for a task and its pledges, solutions, and votes
- **THEN** `task` MUST be a `TaskDetail` object with `pledges`, `solutions`,
  `votesBySolution`, and `payout` populated from the received events

#### Scenario: Status is derived, not stored

- **WHEN** the `TaskDetail` is composed
- **THEN** `status` MUST be computed by calling `deriveTaskStatus()` with the
  raw events — it MUST NOT be read from a tag on the task event
...
#### Scenario: Loading a new task replaces the previous

- **WHEN** `load()` is called while a previous task is loaded
...
- **WHEN** votes arrive for multiple solutions of the same task

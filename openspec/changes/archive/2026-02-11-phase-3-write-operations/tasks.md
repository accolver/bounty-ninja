...
- [x] 1.1 Create `src/lib/task/blueprints.ts` with EventFactory blueprint
...
- [x] 2.2 Create `src/lib/cashu/mint.ts` with singleton `CashuMint` and
      ...
      operation. Support per-task mint override via `["mint", "<url>"]` tag.
...
- [x] 5.1 Create `src/lib/components/task/TaskForm.svelte` — form
      ...
      future.
- [x] 5.2 Implement d-tag generation in `TaskForm.svelte` — URL-safe slug from
      title + random suffix (e.g., `lightning-vending-machine-a1b2c3`) for
      NIP-33 uniqueness.
- [x] 5.3 Wire TaskForm submission to blueprints + EventFactory: construct
      ...
      publish to relays, success toast "Task created!".
- [x] 5.4 Implement post-creation navigation — encode new task's NIP-33
      address as `naddr` via `nip19.naddrEncode()` from `nostr-tools`, navigate
      to `/task/<naddr>` using SvelteKit `goto()`.
- [x] 5.5 Create `src/routes/task/new/+page.svelte` — renders `TaskForm` for
      authenticated users. Show "Sign in with a Nostr extension to create a
      task" prompt with `LoginButton` for unauthenticated users.

## 6. Pledge Flow

- [x] 6.1 Create `src/lib/components/pledge/PledgeButton.svelte` — "Fund this
      task" CTA on task detail page. Opens PledgeForm dialog when clicked by
      authenticated user. Shows sign-in prompt for unauthenticated users.
      Disabled with tooltip "This task is no longer accepting pledges" when
      task status is `completed`, `expired`, or `cancelled`.
- [x] 6.2 Create `src/lib/components/pledge/PledgeForm.svelte` — dialog/modal
      ...
      this task creator." — user must acknowledge before submit activates.
      Enforce single-mint per task (reject if mint mismatch).
- [x] 6.3 Wire PledgeForm submission: create P2PK-locked Cashu token via
      `src/lib/cashu/escrow.ts` (locked to task creator's pubkey), construct
...
- [x] 6.4 Verify task status transitions reactively — first pledge on a
      `draft` task transitions derived status to `open`, `TaskStatusBadge`
...
- [x] 7.1 Create `src/lib/components/solution/SolutionForm.svelte` — form
      ...
      URL, anti-spam fee payment. Visible only when task status is `open` or
...
- [x] 7.2 Implement anti-spam fee validation in SolutionForm — if task has
...
- [x] 7.4 Verify task status transitions — first solution on an `open` task
      transitions derived status to `in_review`, `TaskStatusBadge` updates,
...
- [x] 8.1 Create `src/lib/components/voting/VoteButton.svelte` — renders
      ...
      with tooltip "Sign in and fund this task to vote" for unauthenticated
...
- [x] 8.2 Implement voting eligibility enforcement — check EventStore for Kind
      73002 pledge events matching voter's pubkey and task address. Votes from
...
- [x] 8.5 Verify real-time tally updates — new Kind 1018 events from relay
      ...
      task creator." and show "Trigger Payout" button for creator.

## 9. Payout Orchestration

- [x] 9.1 Implement payout trigger visibility on task detail page
      (`src/routes/task/[naddr]/+page.svelte`) — "Trigger Payout" button
      visible only when: current user is task creator AND a solution has
      reached consensus approval. Hidden for non-creators (show "Awaiting payout
      from task creator") and before consensus.
...
- [x] 9.3 Implement pledge token collection — query EventStore for all Kind
      73002 events with matching `["a", "<task-address>"]`, extract and decode
...
- [x] 9.6 Wire payout publishing: construct Kind 73004 event with required tags
      ...
      solver!". Task status transitions to `completed`. Ignore Kind 73004
      events from non-creator pubkeys.
- [x] 9.7 Implement solver claim notification — when solver views task detail
...
- [x] 10.2 Create `src/lib/components/auth/ProfileMenu.svelte` — logged-in user
      ...
      Tasks", "Create Task", and "Sign Out" action.
...
- [x] 11.2 Update `src/routes/task/[naddr]/+page.svelte` — integrate
...
- [x] 13.1 Create `src/tests/integration/task-store.test.ts` — test EventStore
      ...
      parseTaskDetail, add Kind 73001 → verify `solutionCount` updates, add
...
- [x] 13.2 Create `src/tests/integration/pledge-flow.test.ts` — test pledge
      ...
      event, verify EventStore insert, verify task status transition from

...
to Nostr relays; pledges use Cashu ecash with P2PK locks and pledger-controlled
manual releases. The app is a static client-side SvelteKit SPA deployed to
Cloudflare Pages.
...
- Production payment writes default to disabled through
  `PUBLIC_PAYMENT_WRITES_ENABLED`. Do not bypass this gate or imply payments are
  approved for real funds.
- The supported flow is a manual Minibits handoff. Pledges are exact-amount,
  DLEQ-verified P2PK tokens locked to the pledger's dedicated Cashu payment key.
- Every proof must match the exact mint/keyset and NUT-11 policy: unit `sat`,
  unspent and unique, no locktime or refund keys, and exactly one permanently
  required `SIG_INPUTS` signature.
- After consensus, each pledger manually creates one exact solver-locked output
  per source pledge. The app does not swap tokens, sign proofs, or force release.
...
- `src/lib/cashu/escrow.ts` uses a **pledger-controlled escrow model**: pledges
  are locked to the pledger's dedicated Cashu payment key; after consensus each
  pledger manually releases their portion to the solver's declared payment key.

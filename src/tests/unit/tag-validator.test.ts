...
describe('validateEventTags — Kind 7302 (Pledge)', () => {
	it('rejects pledge missing a tag', () => {
		const event = makeEvent(7302, [
			['amount', '100'],
	...
	it('rejects pledge with invalid mint URL', () => {
		...
		expect(result.errors).toContainEqual(
			expect.stringContaining("'mint' tag must be a valid HTTPS URL")
		);
	});
...
describe('validateEventTags — Kind 7301 (Solution)', () => {
	it('rejects solution missing a tag', () => {
		const event = makeEvent(7301, [], 'My solution content');
		const result = validateEventTags(event);
		expect(result.valid).toBe(false);
		expect(result.errors).toContainEqual(expect.stringContaining("'a' tag"));
	});
	...
	it('rejects solution with invalid a tag format', () => {
		const event = makeEvent(7301, [['a', 'bad-format']], 'My solution content');
		const result = validateEventTags(event);
		expect(result.valid).toBe(false);
		expect(result.errors).toContainEqual(expect.stringContaining('invalid format'));
	});
	...
	it('rejects solution with empty content', () => {
		const event = makeEvent(7301, [['a', VALID_A_TAG]], '');
		const result = validateEventTags(event);
		expect(result.valid).toBe(false);
	...
	it('accepts valid solution event', () => {
		const event = makeEvent(7301, [['a', VALID_A_TAG]], 'Here is my solution.');
		const result = validateEventTags(event);
		expect(result.valid).toBe(true);
		expect(result.errors).toHaveLength(0);
	});
...
describe('validateEventTags — Kind 7304 (Payout)', () => {
	it('rejects payout missing a tag', () => {
		const event = makeEvent(7304, [
			['e', HEX_EVENT_ID],
			['p', HEX_PUBKEY]
		]);
		const result = validateEventTags(event);
	...
	it('rejects malformed, uppercase, wrong-scheme, and duplicate payment tags', () => {
		...
		for (const paymentTags of invalidTags) {
			expect(validateEventTags(makeEvent(7302, [...base, ...paymentTags])).valid).toBe(false);
		}
	});

	it('rejects malformed or multiple source pledge references', () => {
		const malformed = makeEvent(7304, [
			['a', VALID_A_TAG],
			['e', HEX_EVENT_ID, '', 'solution'],
		...
		const duplicate = makeEvent(7304, [
			['a', VALID_A_TAG],
			['e', HEX_EVENT_ID, '', 'solution'],
			['e', 'd'.repeat(64), '', 'source'],
	...
	it('requires source and mint tags to appear together', () => {
		const sourceOnly = makeEvent(7304, [
			['a', VALID_A_TAG],
			['e', HEX_EVENT_ID, '', 'solution'],
			['e', 'd'.repeat(64), '', 'source'],
			['p', HEX_PUBKEY]
		]);
		const mintOnly = makeEvent(7304, [
			['a', VALID_A_TAG],
			['e', HEX_EVENT_ID],
			['p', HEX_PUBKEY],
			['mint', VALID_MINT_URL]

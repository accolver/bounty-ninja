...
{#if !dismissed}
	<div class="border-t border-border pt-5 pb-8">
		<div class="flex items-start justify-between">
			<h2 class="text-lg font-semibold text-foreground">
				How bounty<span class="text-primary">.ninja</span> works
			</h2>
			<button
				onclick={dismiss}
		...
		<div class="mt-5 grid grid-cols-1 gap-6 sm:grid-cols-3">
			<div class="flex flex-col items-center gap-3 text-center">
				<div
					class="flex h-12 w-12 items-center justify-center rounded-full border border-primary bg-transparent"
				>
					<Zap class="h-6 w-6 text-primary" />
				</div>
				<h3 class="text-sm font-semibold text-foreground">1. Post a Bounty</h3>
				<p class="text-xs text-muted-foreground">
					Describe your bounty and set a reward in sats (small units of Bitcoin — think of them like
					cents to a dollar).
				</p>
			</div>
		...
		<div class="mt-5 pl-4 border-l-2 border-border py-3">
			<p class="text-xs text-muted-foreground">
				<strong class="text-foreground">Completely anonymous.</strong> No email, no accounts, no
				personal data. Sign in with a cryptographic key — we never know who you are. Built on
				<a
		...
		<div class="mt-4 text-center">
			<a
				href="/about"
				class="text-xs text-primary underline transition-colors hover:text-primary/80"
			>

<script lang="ts">
	...
	async function handleExtensionLogin() {
		if (!signerState.available && !window.nostr) {
			// No extension — show safe signer installation guidance.
		...
		>
			{#if showBunkerForm}
				...
				<div class="space-y-3">
					...
					<p class="text-xs text-muted-foreground">
						Paste a <code class="rounded bg-muted px-1">bunker://</code> URI from your remote signer (e.g.
						Amber, Signet, nsecBunker).
					</p>
			...
			{:else if showInstallLinks}
				...
				<div class="space-y-3">
					...
					<p class="text-xs text-muted-foreground">
						Install a browser extension to sign in securely. These manage your identity so you never
					...
					<div class="flex flex-col gap-2">
						...
						<a
							...
							class="text-sm text-primary underline transition-colors hover:text-primary/80"
						>
							Alby <span class="text-muted-foreground"
								>(Chrome, Firefox) — also handles Bitcoin payments</span
							>
						</a>
					</div>
				</div>
			{:else}
				...
				>
					...
					<div class="text-left">
						...
						<div class="text-xs text-muted-foreground">
							Recommended — most secure sign-in method
						</div>
					</div>
				</button>
				...
				>
					...
					<div class="text-left">
						...
						<div class="text-xs text-muted-foreground">
							NIP-46 — keys stay on your signer device
						</div>
					</div>
				</button>

<script lang="ts">
	...
	const overflowCount = $derived(Math.max(0, task.tags.length - 3));
</script>
...
>
	...
	{#if visibleTags.length > 0}
		<div class="flex flex-wrap gap-1.5 px-4 pb-3" aria-label="Task tags">
	...
	<div class="mt-auto border-t border-border/50 px-4 pt-3 pb-4">
		...
		{#if task.solutionCount > 0}
			<div class="mt-2 text-xs text-muted-foreground">
				{task.solutionCount}
				{task.solutionCount === 1 ? 'solution' : 'solutions'} submitted
			</div>
		{/if}
	</div>
</a>

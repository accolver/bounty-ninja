...
[github.com/Spl0itable/nosflare](https://github.com/Spl0itable/nosflare) — A serverless Nostr relay built on:
...
- **Durable Objects** — long-lived WebSocket connections, multi-region mesh (9 locations) for real-time event broadcast
...
The missing NIP-50 is the biggest gap but it's on their roadmap and we can work around it. Everything else is a strong fit.
...
Dramatically cheaper than running a VPS relay (strfry/nostr-rs-relay) with equivalent global performance.

---

## Implementation Steps

1. **Fork Nosflare** → `soveng/nosflare` or `soveng/bounty-relay`
...
5. **Client update** — add dedicated relay as primary, adjust publish/subscribe logic
6. **Test** — verify event round-trip, WebSocket stability, D1 replication
7. **Monitor** — CF analytics dashboard for request volume, error rates, DO billing
...
- Add NIP-42 auth for premium features (priority relay access for bounty posters)

---

## Verdict

**Nosflare is an excellent fit.** Same Cloudflare account, same toolchain, serverless scaling, global edge performance, and ~$5-30/mo at reasonable scale. The main gap (NIP-50 search) is workable — client-side search handles the near term, and we can contribute search support upstream.

Recommend deploying as the **first scaling investment** once we have real users posting bounties.

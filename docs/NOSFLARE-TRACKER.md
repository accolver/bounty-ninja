...
Deploy a dedicated Nostr relay at `relay.bounty.ninja` using Nosflare on our
...
- [ ] Update `connectDefaultRelays()` — dedicated relay first, public relays as
      fallback
- [ ] Publish logic: write to dedicated + 2 public relays for redundancy
- [ ] Read logic: query dedicated first, fan out only if needed or for profile
      metadata
...
- CF Account ID: set via `CLOUDFLARE_ACCOUNT_ID` env var
